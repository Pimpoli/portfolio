// js/store.js - Tienda estilo Proyectos integrada con YouTube y Gumroad

document.addEventListener('DOMContentLoaded', async () => {
  const storeSection = document.getElementById('store-container');
  ...
  const STORE_PRODUCTS = [
    {
      // Extrae solo las letras/números finales de tu video de YouTube (ej. dQw4w9WgXcQ)
      youtubeId: 'jNQXAC9IVRw', // <-- Cámbialo por el ID del video donde explicas tu producto
      gumroadUrl: 'https://store.pimpolidev.com/l/wyfdgk', // Tu link de Gumroad real
      price: '$5.00 USD' // Pon el precio manualmente aquí
    },
    {
      // Puedes borrar este segundo producto de prueba si no lo necesitas aún
      youtubeId: 'dQw4w9WgXcQ', 
      gumroadUrl: 'https://store.pimpolidev.com/l/ejemplo2',
      price: '$10.00 USD'
    }
  ];

  // Usamos la misma clave de API que en tus proyectos para sacar los datos de YouTube
  ...
  mainGrid.className = 'projects-grid'; // Usamos tu clase de CSS que los ordena de 2 en 2 perfectamente

  const moreGrid = document.createElement('div');
  moreGrid.id = 'more-store';
  moreGrid.className = 'more-content';
  ...
  function renderStoreUI() {
    ...
    loadedProducts.forEach((p, index) => {
      ...
      // Aplicamos el estilo exacto de las tarjetas de proyectos
      ...
      if (p.desc && p.desc.trim() !== '') {
        ...
        // flex-grow asegura que los botones queden alineados abajo si los textos tienen distinto tamaño
  ...
  // ==========================================
  // 3. MODAL DE COMPRA Y DETALLES (Ventana Emergente)
  // ==========================================
  ...
  function openStoreModal(p) {
    ...
    // Cargar Descripción desde YouTube
    if (p.desc && p.desc.trim() !== '') {
    ...
    // Configurar Botón Verde de Compra
  ...
  function closeStoreModal() {
    ...
    if (overlay) {
      overlay.style.display = 'none';
      videoContainer.innerHTML = ''; // Detiene el video al cerrar
      document.body.classList.remove('modal-open');
    }
  }

  // Delegación de clics: Al tocar cualquier tarjeta se abre el modal
  storeSection.addEventListener('click', (e) => {
    const card = e.target.closest('.project-card');
    if (!card) return;
    const p = JSON.parse(card.dataset.productData);
    
    // Ignoramos clics si están tocando directamente enlaces del overlay de Gumroad
    if (e.target.closest('a[data-gumroad-overlay-checkout]')) return;
    
    openStoreModal(p);
  });

  // Re-renderizar si cambian de idioma para actualizar el texto de los botones
  document.addEventListener('languageLoaded', () => {
    // Repintamos la interfaz sin volver a descargar datos de YouTube
    renderStoreUI();
  });
